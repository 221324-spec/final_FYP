...
/** Axios / network errors often stringify as "Error" — log full context for ops. */
function formatMlServiceError(err) {
  if (!err || typeof err !== 'object') return String(err);
  const bits = [err.message || String(err)];
  if (err.code) bits.push(`code=${err.code}`);
  if (err.response != null) {
    bits.push(`http=${err.response.status}`);
    const d = err.response.data;
    if (d != null) {
      bits.push(typeof d === 'string' ? d.slice(0, 400) : JSON.stringify(d).slice(0, 400));
    }
  }
  if (err.cause && err.cause.message) bits.push(`cause=${err.cause.message}`);
  return bits.join(' | ');
}
...
async function inferEmotionFromRecentSignals(patientId) {
  ...
  if (!entries.length) {
    return {
      ...
      modelVersion: 'contextual-fallback-v2',
      analysisSource: 'fallback_contextual',
      note: 'No recent mood logs; defaulted to neutral baseline.'
    };
  }

  // moodValue & craving are on ~1–10 scales (see MoodEntry schema)
  ...
  const latestMood = Number(latest.moodValue || 0);
  const latestCraving = Number(latest.craving || 0);
  // Blend: recent check-in matters more than week-old data
  const blendedMood = latestMood * 0.55 + avgMood * 0.45;
  const blendedCraving = latestCraving * 0.55 + avgCraving * 0.45;
  ...
  // High craving / low mood → anxious (distress + urge pattern)
  if (blendedCraving >= 7.5 || blendedMood <= 2.2 || (blendedCraving >= 6 && blendedMood <= 4)) {
    emotion = 'anxious';
    intensityScore = Math.min(0.88, 0.45 + blendedCraving / 14 + (10 - blendedMood) / 35);
  } else if (blendedMood <= 4.2 && blendedCraving < 6.5) {
    emotion = 'sad';
    intensityScore = Math.min(0.85, 0.38 + (5 - blendedMood) / 12);
  } else if (blendedMood >= 6.8 && blendedCraving <= 5.5) {
    emotion = 'happy';
    intensityScore = Math.min(0.85, 0.36 + blendedMood / 14);
  } else if (blendedMood >= 5.5 && blendedCraving <= 6) {
    emotion = 'neutral';
    intensityScore = 0.32 + stdDev * 0.06;
  } else {
    intensityScore = 0.3 + stdDev * 0.08;
  }
...
exports.createScan = async (req, res) => {
  try {
    ...
    try {
      ...
      console.error(
        `[AI Mood ML] ${formatMlServiceError(mlErr)} (url=${AI_MOOD_ML_URL})`
      );
