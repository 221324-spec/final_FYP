...
if _PYTHON_VERSION < _MIN_PYTHON:
    print(f"ERROR: Python {_PYTHON_VERSION.major}.{_PYTHON_VERSION.minor} is too old.")
    print(f"       Minimum required: Python {_MIN_PYTHON[0]}.{_MIN_PYTHON[1]}")
    print(f"       Recommended: Python {_RECOMMENDED_PYTHON[0]}.{_RECOMMENDED_PYTHON[1]}")
    sys.exit(1)
elif _PYTHON_VERSION >= (3, 13):
    print(f"WARNING: Python {_PYTHON_VERSION.major}.{_PYTHON_VERSION.minor} may have compatibility issues.")
    print(f"         Recommended version: Python {_RECOMMENDED_PYTHON[0]}.{_RECOMMENDED_PYTHON[1]}")
elif (_PYTHON_VERSION.major, _PYTHON_VERSION.minor) != _RECOMMENDED_PYTHON:
    print(f"INFO: Python {_PYTHON_VERSION.major}.{_PYTHON_VERSION.minor} is supported.")
    print(f"      Recommended: Python {_RECOMMENDED_PYTHON[0]}.{_RECOMMENDED_PYTHON[1]}")
...
if __name__ == '__main__':
    ...
    # Prefer ML_PORT: Node often sets PORT=5000 for the API; child inherits it and must not steal the ML bind.
    port = int(os.environ.get('ML_PORT') or os.environ.get('PORT') or 5001)
