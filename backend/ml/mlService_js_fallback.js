...
const { all: textTrainingData } = loadTextDataset();
const { all: emotionTrainingSeedData } = loadEmotionDataset();
const { all: riskTrainingData } = loadRiskDataset();

const emotionTrainingData = dedupeSamples(
  [...textTrainingData, ...emotionTrainingSeedData],
  (sample) => `${sample.emotion || 'neutral'}|${sample.text}`
);
...
async function trainAll() {
  ...
  for (const sample of emotionTrainingData) {
  ...
  const emAccuracy = +(emCorrect / emotionTrainingData.length * 100).toFixed(1);
  console.log(`    ✓ Emotion classifier trained — accuracy: ${emAccuracy}% (${emotionTrainingData.length} samples)`);
  ...
  modelMeta.emotionClassifier = {
    trained: true,
    trainedAt: new Date().toISOString(),
    samplesUsed: emotionTrainingData.length,
    accuracy: emAccuracy,
  };
...
function classifyText(text) {
  ...
  const risk = riskTextClassifier.classify(normalizedText);
  const emotion = emotionTextClassifier.classify(normalizedText);

  // Get confidence from classifications
  const riskClassifications = riskTextClassifier.getClassifications(normalizedText);
  ...
  const emotionClassifications = emotionTextClassifier.getClassifications(normalizedText);
...
function dedupeSamples(records, keyFn) {
  const seen = new Set();
  const output = [];
